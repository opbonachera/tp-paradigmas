package FormatoDeTexto;

public class Formateo {
	public static final String RESET = "\033[0m";
    public static final String NEGRITA = "\033[1m";
    public static final String CURSIVA = "\033[3m";

    public static final String ROJO_NEGRITA = "\033[1;31m";
    public static final String VERDE_NEGRITA = "\033[1;32m";
    public static final String AMARILLO_NEGRITA = "\033[1;33m";
    public static final String AZUL_NEGRITA = "\033[1;34m";
    public static final String VIOLETA_NEGRITA = "\033[1;35m";
    public static final String GRIS_CLARITO_CURSIVA = "\033[3;37m";
    public static final String AMARILLO_CURSIVA = "\033[3;33m";
    public static final String VERDE_CURSIVA = "\033[3;32m";
    public static final String ROJO_CURSIVA = "\033[3;31m";
    public static final String VERDE_FLUOR = "\033[1;92m";
    public static final String AZUL_OSCURO_NEGRITA = "\033[1;34m";
    public static final String DORADO_NEGRITA_CURSIVA = "\033[1;3;33m";
}
